 ...
 */
package org.openintents.safecloud.client.helper;

import org.openintents.safecloud.client.OISafeCloud;
import org.openintents.safecloud.client.event.RPCInEvent;
import org.openintents.safecloud.client.event.RPCOutEvent;
import org.openintents.safecloud.shared.exception.NotLoggedInException;
...
public abstract class RPCCall<T> implements AsyncCallback<T> {
  ...
  private void onRPCIn() {
    OISafeCloud.get().getEventBus().fireEvent(new RPCInEvent());
  }

  private void onRPCOut() {
    OISafeCloud.get().getEventBus().fireEvent(new RPCOutEvent());
  }
